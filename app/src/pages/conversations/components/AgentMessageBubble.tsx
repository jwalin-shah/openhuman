...
// ⚡ Bolt: Wraps expensive React Markdown rendering in React.memo()
// This optimization prevents costly re-parsing and re-rendering of entire chat histories
// on every keypress or new token received in the parent Conversations view.
export const BubbleMarkdown = memo(
  ({ content, tone = 'agent' }: { content: string; tone?: 'agent' | 'user' }) => {
    const proseTone =
      tone === 'user'
...
// ⚡ Bolt: Wraps Markdown table cell rendering in React.memo()
// Avoids re-parsing Markdown tables when parent re-renders unnecessarily.
...
// ⚡ Bolt: Wraps individual message bubbles in React.memo()
// Ensures that previously rendered message segments aren't re-evaluated
// when the overall conversation updates.
export const AgentMessageBubble = memo(
  ({ content, position = 'single' }: { content: string; position?: AgentBubblePosition }) => {
